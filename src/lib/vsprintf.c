...
#define SPECIAL 0x20 // 0x（8进制输出0，16进制输出0x）
#define SMALL 0x40   // 使用小写字母
#define DOUBLE 0x80  // 浮点数

#define is_digit(c) ((c) >= '0' && (c) <= '9') //判断是否属于数字
...
static char *number(char *str, u32 *num, int base, int size, int precision, int flags)
{
    ...
    // 如果 flags 指出要填零，则置字符变量 PAD='0'，否则 PAD 等于空格字符
    pad = (flags & ZEROPAD) ? '0' : ' ';
    ...
    // 若 flags 中没有左调整（左对齐）标志, 则在剩余宽度中存放 PAD 字符（'0'或空格）
    if (!(flags & LEFT))
        while (size-- > 0)
            *str++ = pad;
